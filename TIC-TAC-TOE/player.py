...
def minimax(self, state, player):
    ...
    if player == max_player:
        best = {'position': None, 'score': -math.inf} # every score should me maximize (be larger)
    else:
        best = {'position': None, 'score': math.inf} # every score should me minimize
        
    for possible_move in state.available_movements():
        ...
        sim_score['position'] = possible_move #otherwise this will get messed up from the recurision
        
        # step 4: update the dictionaries if necessary
        if player == max_player:
            if sim_score['score'] > best['score']:
                best = sim_score # replace best
        else: # but minimi the other player
            if sim_score['score'] < best['score']:
                best = sim_score # replace best
    return best